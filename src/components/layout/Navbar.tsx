        ...
        <nav className="hidden items-center gap-8 md:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-[#0E8FA3] transition-colors hover:text-[#0c7689]"
            >
              {link.label}
            </Link>
          ))}
          ...
          <nav className="flex flex-col gap-1">
            {links.map((link) => (
              <Link
                ...
                className="rounded-lg px-3 py-2.5 text-sm font-medium text-[#0E8FA3] hover:bg-zinc-50"
              >
                {link.label}
              </Link>
            ))}
